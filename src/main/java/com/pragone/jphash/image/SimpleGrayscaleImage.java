...
public class SimpleGrayscaleImage {
	...
	public void blur() {
		...
		for (int y = 0; y < height; y++) {
			...
			for (int x = 2; x < width - 2; x++) {
				t = (int) (((long) (buffer_2[x - 2] + buffer_1[x - 1] + buffer[x] + buffer_1[x + 1] + buffer_2[x + 2]))
						* 409) >> 10; // 1024/409 = 2.503 ~ 1 + 2*0.5 + 2*0.25
		...
		for (int x = 0; x < width; x++) {
			...
			// y = 1
			this.data.put(width + x, (byte) (((buffer_1[0] + buffer[1] + buffer_1[2] + buffer_2[3]) * 455) >> 10)); // 1024/455
																													// =
																													// 2.250549
			for (int y = 2; y < height - 2; y++) {
				long t = (((long) (buffer_2[y - 2] + buffer_1[y - 1] + buffer[y] + buffer_1[y + 1] + buffer_2[y + 2]))
						* 409) >> 10;
				this.data.put(width * y + x, (byte) (t > 255 ? 255 : t)); // 1024/409 = 2.503 ~ 1 + 2*0.5 + 2*0.25
			}
			int y = height - 2;
			this.data.put(width * y + x,
					(byte) (((buffer_2[y - 2] + buffer_1[y - 1] + buffer[y] + buffer_1[y + 1]) * 455) >> 10)); // 1024/455
																												// =
																												// 2.250549
			y++;
			this.data.put(width * y + x, (byte) (((buffer_2[y - 2] + buffer_1[y - 1] + buffer[y]) * 585) >> 10)); // 1024/585
																													// =
																													// 1.75042
		}
	}
	...
	public void loadImage(BufferedImage image) {
		...
		if (maxPixel > 0) {
			...
			// for (int i =0; i < numPixels; i++) {
			// long temp = (this.data.get(i) * NORMALIZATION_APROX[this.maxPixel]) >>
			// NORMALIZATION_DENOMINATOR_POWER;
			// this.data.put(i, (byte) (temp & 0xFF));
			// }
		}
